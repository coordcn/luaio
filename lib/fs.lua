...
fs.FILE = fs_native.FILE
fs.DIR = fs_native.DIR
fs.LINK = fs_native.LINK
fs.FIFO = fs_native.FIFO
fs.SOCKET = fs_native.SOCKET
fs.CHAR = fs_native.CHAR
fs.BLOCK = fs_native.BLOCK

-- @example: local ret = fs.access(path[, mode])
-- @param: path {string}
-- @param: mode {string|defualt: 0}
...
function fs.access(path, mode)
  return fs_native.access(path, mode or 0)
end
...
-- @param: path {string}
-- @param: flag {string|default: 'r'}
-- @param: mode {integer|default: 0666}
-- @return: ret {integer}  if ret < 0 ret is errno else ret is fd
function fs.open(path, flag, mode)
  return fs_native.open(path, flag or 'r', mode or 438)
end
...
function fs.read(fd, buffer, offset)
  return fs_native.read(fd, buffer, offset or -1)
end

-- @example: local data, ret = fs.readFile(path[, flag, mode])
-- @param: path {string}
-- @param: flag {string|defualt: 'r'}
-- @param: mode {integer|defualt: 0666}
-- @return: data {string}
-- @return: ret {integer} if ret < 0 ret is errno else ret is read bytes
function fs.readFile(path, flag, mode)
  local fd = fs_native.open(path, flag or 'r', mode or 438)
  ...
  if stat.type ~= fs.FILE then
    fs_native.close(fd)
    return nil, ERRNO.LUAIO_ENOTFILE
  end
...
-- @param: path {string}
-- @param: data {string|buffer|table[array(string|buffer)]}
-- @param: flag {string|defualt: 'a'}
-- @param: mode {integer|default: 0666}
-- @return: ret {integer} if ret < 0 ret is errno else ret is write bytes
function fs.appendFile(path, data, flag, mode)
  local fd = fs_native.open(path, flag or 'a', mode or 438)
  if fd < 0 then return fd end
  return fs_native.write(fd, data, -1)
end
...
-- @param: path {string}
-- @param: mode {integer|defualt: 511}
-- @return: err {integer}
function fs.mkdir(path, mode)
  return fs_native.mkdir(path, mode or 511)
end

-- @example: local err = fs.mkdirp(path[, mode])
-- @param: path {string}
-- @param: mode {integer}
-- @return: err {integer}
function fs.mkdirp(path, mode)
  local ret = fs_native.mkdir(path, mode)
  if ret == ERRNO.UV_EEXIST then
    local stat, err = fs_native.stat(path)
    if err < 0 then return err end
    if stat.type ~= fs.DIR then
      return ERRNO.UV_ENOTDIR
    end
  elseif ret == ERRNO.UV_ENOENT then
    local err = fs.mkdirp(path_join(path, '..'), mode)
    if err < 0 then return err end
...
-- @param: path {string}
-- @param: length {integer|defualt: 0}
-- @return: err {integer}
function fs.truncate(path, length)
  local fd = fs_native.open(path, 'r+', 438)
  if fd < 0 then return fd end
  local ret = fs_native.ftruncate(fd, length or 0)
  fs.native.close(fd)
  return ret
end
...
function fs.ftruncate(fd, length)
  return fs_native.ftruncate(fd, length or 0)
end
